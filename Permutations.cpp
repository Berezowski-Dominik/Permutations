...
void get_pins(string observed)
{
    int combinations = 1;                           //  how many combination we have

    vector <vector <char>> NumberandNeighbours{
        {'0','8'},                                  //  0   
        {'1','2','4'},                              //  1
        {'1','2','3','5'},
        {'2','3','6'},
    ...
    for (int i = 0; i < observed.length(); i++)     //  search many combinations
        combinations *= NumberandNeighbours[observed[i]-48].size();

    vector <string> ReturnCombinations (combinations);
    ...
    for (int i = 0; i < observed.length(); i++)
    {
        ...
        {
            if (combinations / (NumberandNeighbours[observed[i] - 48].size() * (1 + option)) == j)
                option++;
                
            ReturnCombinations[j] += NumberandNeighbours[observed[i] - 48][option];
        }
    }
